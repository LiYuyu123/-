...
class MDatabase
{
    ...
    public function connect( $driver = null, $server = null, $username = null, $password=null, $database=null, $charset = 'utf8' )
    {
        ...
        if($driver == "mysql"){
            try{
                $this->_db = new PDO("mysql:host={$server};" . ($database != null ? "dbname={$database}" : ""), $username, $password);
                $this->clearError();
                return true;
            }
            catch (PDOException $e) {
                $this->_lastSql = "[DNS]={$this->_driver}:Server={$this->_server}, [USER]={$username}, [PASSWORD]={******}";
